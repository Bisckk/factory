...
import { Wrench, ClipboardList, Filter, Search, Plus, MoreHorizontal, Clock, Eye, User, Phone, Mail, X, Check, Hash } from 'lucide-react';
...
import { ClientDetailDrawer } from '@/components/dashboard/orders/client-detail-drawer';
...
export default function OrdersPage() {
    ...
    const [mainView, setMainView] = useState<'orders' | 'clients'>('orders');
    ...
    const MOCK_CLIENTS = [
        { id: "c_1", name: "Carlos Martínez", phone: "300 123 4567", email: "carlos@ejemplo.com", registered: "10 Feb 2025", active_orders: 3 },
        { id: "c_2", name: "Andrea López", phone: "310 987 6543", email: "andrea@ejemplo.com", registered: "15 Ene 2025", active_orders: 1 },
        { id: "c_3", name: "Diego Ramírez", phone: "320 456 7890", email: "-", registered: "05 Mar 2025", active_orders: 0 },
        { id: "c_4", name: "Sofía Hernández", phone: "315 222 3344", email: "sofia@ejemplo.com", registered: "20 Feb 2025", active_orders: 5 },
        { id: "c_5", name: "Juan Pérez", phone: "301 555 6677", email: "juan@ejemplo.com", registered: "02 Mar 2025", active_orders: 2 },
    ];

    const [statusFilters, setStatusFilters] = useState<OrderStatus[]>([]);

    // ─── Client order count filter ───
    ...
    const matchesOrderCount = (count: number, range: OrderCountRange) => {
        switch (range) {
    ...
    const filteredClients = MOCK_CLIENTS.filter((client) => {
        if (orderCountFilters.length > 0 && !orderCountFilters.some(r => matchesOrderCount(client.active_orders, r))) return false;
        if (search) {
            const query = search.toLowerCase();
            return (
        ...
        <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-zinc-500">
                        {isMechanic
                            ...
                            : 'Gestiona los servicios activos, históricos y la base de clientes del taller.'
                        }
                    </p>
                </div>
                {canCreate && (
                    ...
                    >
                        <Plus className="h-4 w-4" /> {mainView === 'orders' ? 'Nueva Orden' : 'Nuevo Cliente'}
                    </button>
                )}
            </header>

            {!isMechanic && (
                <div className="flex p-1 bg-zinc-900 border border-zinc-800 rounded-xl w-full sm:w-fit mb-4">
                    <button
                        onClick={() => setMainView('orders')}
                        className={cn(
                            "flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 text-xs font-bold rounded-lg transition-all",
                            mainView === 'orders' ? "bg-[#141417] text-white shadow-sm border border-zinc-700" : "text-zinc-500 hover:text-zinc-300"
                        )}
                    >
                        <ClipboardList className="h-4 w-4" /> Órdenes Activas
                    </button>
                    <button
                        onClick={() => setMainView('clients')}
                        className={cn(
                            "flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 text-xs font-bold rounded-lg transition-all",
                            mainView === 'clients' ? "bg-[#141417] text-white shadow-sm border border-zinc-700" : "text-zinc-500 hover:text-zinc-300"
                        )}
                    >
                        <User className="h-4 w-4" /> Gestión de Clientes
                    </button>
                </div>
            )}
            ...
            <AnimatePresence mode="wait">
                {mainView === 'orders' ? (
                    ...
                    <motion.div key="clients" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                        <div className="rounded-xl border border-zinc-800 bg-[#141417] overflow-hidden">
                            {/* Toolbar */}
                            <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-b border-zinc-800 gap-4">
                                <div className="relative w-full sm:max-w-xs">
                                ...
                                <table className="w-full text-left text-sm whitespace-nowrap">
                                    <thead className="bg-zinc-900/50 text-xs uppercase tracking-widest text-zinc-500">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 font-semibold border-x border-zinc-800/50 text-center">Órdenes</th>
                                                ...
                                                <motion.tr key={client.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col gap-1">
                                                            <span className="flex items-center gap-1.5 text-xs text-zinc-400"><Phone className="h-3 w-3" /> {client.phone}</span>
                                                        </div>
                                                    </td>
